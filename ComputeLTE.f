	subroutine ComputeLTE(datafile,iTr,Aul,Bul,Blu,nu0,n_low,n_up,mol_mass)
	use Parameters
	IMPLICIT NONE
	character*500 datafile
	character*10 name
	integer,allocatable :: u(:),l(:)
	integer i,j,nE,nTr,iTr,k
	...
	do j=1,D%nTheta-1
		...
		do k=1,nE
			UT=UT+g(k)*exp(-E(k)/T)
		enddo
		n_up(i,j)=g(u(iTr))*exp(-E(u(iTr))/T)/UT
		n_low(i,j)=g(l(iTr))*exp(-E(l(iTr))/T)/UT
	enddo
	enddo
