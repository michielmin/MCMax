...
c	Estar=pi*Planck(D%Tstar,phot%lam)*D%Rstar**2
	...
	EnergyTot2=0d0
	do i=1,D%nR-1
		if(nexits.ne.0) call tellertje(i,D%nR-1)
		do j=1,D%nTheta-1
...
c				if(tcontact) then
c				EmisDis(i,j)=Planck(C(i,j)%T,phot%lam)*C(i,j)%Kabs*C(i,j)%dens*C(i,j)%V
c				else
c				EmisDis(i,j)=0d0
c				do ii=1,ngrains
c				do iopac=1,Grain(ii)%nopac
c					EmisDis(i,j)=EmisDis(i,j)+Planck(C(i,j)%TP(ii),phot%lam)*C(i,j)%w(ii)*C(i,j)%wopac(ii,iopac)
c     &								*(Grain(ii)%Kabs(iopac,phot%ilam1)*phot%wl1+Grain(ii)%Kabs(iopac,phot%ilam2)*phot%wl2)
c     &								*C(i,j)%dens*C(i,j)%V
c				enddo
c				enddo
c				endif
			else
...
C     close the file and free the unit number
	...
	subroutine MakeFluxImage(fluxcontr,Rmax,IMDIM,nintegrate,angle,lam0)
	use Parameters
	IMPLICIT NONE
	type(RPhiImage) image
	integer IMDIM
	...
	subroutine FITSImageFlux(image,im,IMDIM,Rmax)
	use Parameters
	IMPLICIT NONE
	integer IMDIM,iadd
	type(RPhiImage) image
